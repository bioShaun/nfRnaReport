...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='---create mRNA analysis report---')
    parser.add_argument('mRNA_report_path',
                        help='a dir where include your analysis results')
    parser.add_argument('--pdf', '-pdf', action='store_true',
                        help='print out pdf report')
    parser.add_argument('--html', '-html', action='store_true',
                        help='print out html report')
    parser.add_argument('--part', '-part', action='store_false',
                        help='project whether complete,defalut False')
    ...
    if args.html:
        ...
        html_report.enrichment_analysis(
            args.mRNA_report_path, analysis_modules)
        ...
        html_report.quantification_analysis(
            args.mRNA_report_path, analysis_modules)
        html_report.rseqc_analysis(args.mRNA_report_path, analysis_modules)
        html_report.create_main_page_nav(
            args.mRNA_report_path, analysis_modules)

    if args.pdf:
        pdf_report.create_pdf_report(
            os.path.abspath(args.mRNA_report_path),
            args.part)
